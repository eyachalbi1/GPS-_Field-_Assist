...
enum _SmsPhase { idle, sending, waitingReply, retryWait, done }
...
class _SState extends State<SequentialSmsScreen> {
  ...
  // ── gestion des tentatives (timers optimisés réseau TN) ───────────────────
  static const int _maxAttempts   = 2;    // 2 essais par commande
  static const int _replyTimeoutS = 60;   // 60s attente réponse GPS par essai
  static const int _retryWaitS    = 300;  // 5 min après 2 échecs → retry auto
  int _attemptCount = 0;
  ...
  Future<void> _restoreState() async {
    try {
      ...
      final list =
          List<Map<String, dynamic>>.from(saved['commands'] as List);
      for (int i = 0; i < _cmds.length && i < list.length; i++) {
        final s = list[i];
        if (s['command'] != _cmds[i].text) continue;
        switch (s['status'] as String? ?? 'pending') {
          case 'confirmed': _cmds[i].status = _St.confirmed; break;
          case 'failed':    _cmds[i].status = _St.failed;    break;
          default:          _cmds[i].status = _St.pending;
        }
        _cmds[i].response = s['response'] as String?;
        final st = s['sentAt'];
        if (st != null) {
          _cmds[i].sentAt = DateTime.tryParse(st.toString());
        }
      }
  ...
  void _startListening() {
    ...
    _pollTimer?.cancel();
    // ── polling toutes les 10s (optimisé) ────────────────────────────────
    _pollTimer = Timer.periodic(
        const Duration(seconds: 10), (_) => _pollInbox());
  ...
  void _onReplyTimeout() {
    ...
    if (_attemptCount < _maxAttempts) {
      ...
      // ── Tous les essais épuisés → erreur + compte à rebours 5 min ────────
      setState(() {
        _cmds[_idx].status   = _St.failed;
        _cmds[_idx].response =
            'Pas de réponse GPS après $_maxAttempts essais (${_replyTimeoutS}s chacun)';
        _phase     = _SmsPhase.retryWait;
        _retryLeft = _retryWaitS;
      });
      _saveState();
      _toast(
        'GPS ne répond pas après $_maxAttempts essais – retry dans 5 min',
        const Color(0xFFDC143C),
        dur: 5,
      );
      _beginRetryCountdown();
    }
  }
  ...
  // ─────────────────────────────────────────────────────────────────────────
  void _beginRetryCountdown() {
    _retryTicker?.cancel();
    _retryFire?.cancel();
    _retryLeft = _retryWaitS;
    _retryTicker = Timer.periodic(const Duration(seconds: 1), (_) {
      if (!mounted) return;
      setState(() { if (_retryLeft > 0) _retryLeft--; });
    });
    _retryFire = Timer(Duration(seconds: _retryWaitS), _executeRetry);
  }
  ...
  void _executeRetry() {
    ...
    if (!mounted) return;
    setState(() {
      _cmds[_idx].status   = _St.pending;
      _cmds[_idx].response = null;
      _phase               = _SmsPhase.idle;
      _attemptCount        = 0;
    });
    _sendCommand(_idx);
  ...
  void _startSequence() {
    ...
    final first = _cmds.indexWhere(
        (c) => c.status == _St.pending || c.status == _St.failed);
    if (first < 0) {
      _toast('Toutes les commandes déjà confirmées',
  ...
  void _sendNext() {
    ...
    if (next < 0) {
      next = _cmds.indexWhere(
          (c) => c.status == _St.pending || c.status == _St.failed);
    }
    if (next < 0) {
      setState(() => _phase = _SmsPhase.done);
      _saveState();
      _toast('Toutes les commandes confirmées avec succès !',
          const Color(0xFF2ECC71), dur: 6);
      return;
    }
    _idx          = next;
    _attemptCount = 0;
  ...
  Future<void> _sendCommand(int i) async {
    ...
    try {
      ...
      if (mounted) {
        ...
        _saveState();
        _toast('Erreur d\'envoi : $e – retry dans 5 min',
            const Color(0xFFDC143C), dur: 5);
        _beginRetryCountdown();
      }
      return;
    }
    ...
    _toast(
      'Envoyé (essai $_attemptCount/$_maxAttempts) : ${_cmds[i].text}',
      const Color(0xFF3498DB),
      dur: 2,
    );

    _replyTimer?.cancel();
    // ── 40s attente réponse GPS ───────────────────────────────────────────
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text(msg,
          style: const TextStyle(
              color: AppTheme.c1, fontWeight: FontWeight.w500)),
      backgroundColor: bg,
                    ...
                    title: Text(c.text,
                        style: TextStyle(
                            ...
                            decoration: c.status == _St.confirmed
                                ...
                                : null)),
                    subtitle:
                        c.response != null ? Text(c.response!) : null,
  ...
  Color _colFor(_St s) {
    switch (s) {
      case _St.confirmed:    return const Color(0xFF2ECC71);
      case _St.waitingReply: return const Color(0xFFFFA500);
      case _St.failed:       return const Color(0xFFDC143C);
  ...
  IconData _iconFor(_St s) {
    switch (s) {
      case _St.confirmed:    return Icons.check_circle;
      case _St.waitingReply: return Icons.hourglass_empty;
      case _St.failed:       return Icons.error;
  ...
  String _labelFor(_St s) {
    switch (s) {
      case _St.confirmed:    return 'Confirmé';
      case _St.sending:      return 'Envoi…';
      case _St.waitingReply: return 'Attente';
      case _St.failed:       return 'Échec';
                        ...
                        children: [
                          Text(
                            ...
                            style: TextStyle(
                              ...
                              color: _phase == _SmsPhase.retryWait
                                  ? const Color(0xFFDC143C)
                                  : Colors.black87,
                            ),
                          ),
                          if (!allDone && nextPend >= 0)
                            Padding(
                              ...
                              child: Text(
                                _phase == _SmsPhase.retryWait
                                    ? 'Cmd ${nextPend + 1} en échec'
                                    : 'Cmd ${nextPend + 1} / $total',
                                style: const TextStyle(
          ...
          Expanded(
            child: Text(
              allDone
                  ...
                  : _phase == _SmsPhase.retryWait
                      ? 'Retry cmd ${nextPend + 1} dans ${_fmt(_retryLeft)}'
                      : _phase == _SmsPhase.sending ||
                            _phase == _SmsPhase.waitingReply
  ...
  Widget _tile(int i) {
    ...
    switch (c.status) {
      case _St.confirmed:
        ...
        ico    = const Icon(Icons.check_circle,
